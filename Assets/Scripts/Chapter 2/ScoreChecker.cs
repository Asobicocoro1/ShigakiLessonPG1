...
if(); と書いた場合、以下の問題が発生します。

条件式が省略されている: if ステートメントには必ず条件式を記述する必要があります。
ステートメントブロックが省略されている: if ステートメントには、条件が真の場合に実行されるステートメントブロックを記述する必要があります。
これらの問題により、コンパイラは正しくプログラムを解釈できず、エラーが発生します。
